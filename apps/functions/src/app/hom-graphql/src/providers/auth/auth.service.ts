...
import {Strategy, Customer} from '@hom-api/models';
...
export class AuthService {
  ...
  async clearOldTokens(customer: Customer) {
    ...
    return this.customerModel
      ...
      .exec()
      .then(() =>
        this.customerModel.findOne({_id: customer._id.toString()}).exec(),
      );
  }
   ...
   * @param token
   */
  async isValidToken(token: string): Promise<AuthTokenEntityResponse> {
    ...
    let isValid = false;
    if (customer) {
      const strategy = customer.strategies.find(
        (thisStrategy) => thisStrategy.token === token,
      );
      if (strategy) {
        const key = `${strategy.id}:${strategy.type}`;
        const cacheToken = await this.cacheManager.get(key);
        if (cacheToken === token) {
          isValid = true;
        }
      }
